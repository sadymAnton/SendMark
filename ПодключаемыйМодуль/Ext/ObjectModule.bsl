...
Функция ПолучитьЗаполненныйОбъектДокумента1С(Параметры)
	...
	Если ЭтоНакладнаяИлиАкт(Параметры.ДокументДД) Тогда
		...
		Результат = ЗагрузитьЭлектроннуюНакладнуюИлиАктВПТУ(Параметры.ДокументДД, Параметры.Контент);
	...
	ИначеЕсли ЭтоСчетФактура(Параметры.ДокументДД) Тогда
		...
		Если ЭтоСчетФактураПоАгентскойСхеме(Параметры.ДокументДД, Параметры.ПакетДД, деревоКонтентаСчетаФактуры) Тогда
			...
			Результат = ЗагрузитьЭлектронныйСчетФактуруВПТУИСчетФактуру(Параметры.ДокументДД, Параметры.Контент);
...
Процедура ВыполнитьПроверкуГотовностиКОбработке(ДокументДД)
	...
	ПроверитьНаЗаполненность(Договор1СПоДокументу(ДокументДД), "Невозможно определить договор контрагента");
...
Функция ЗагрузитьЭлектроннуюНакладнуюИлиАктВПТУ(ДокументДД, Контент)
	...
	ДокументОбъект.ДоговорКонтрагента		= Договор1СПоДокументу(ДокументДД);
...
Функция ЗагрузитьЭлектронныйСчетФактуруВПТУИСчетФактуру(ДокументДД, Контент)
	
	ПТУОбъект = ЗагрузитьЭлектроннуюНакладнуюИлиАктВПТУ(ДокументДД, Контент);
	ПТУОбъект.Записать();
...
Функция Договор1СПоДокументу(ДокументДД)
	
	списокВидовДоговоров = ТиповойМодульДиадока().МетодСервера("", "ПолучитьСписокВидовДоговоров");
	ВыборкаДоговоров = ТиповойМодульДиадока().МетодСервера("", "ВыбратьОсновнойДоговорКонтрагента", Контрагент1СПоДокументу(ДокументДД), Организация1СПоДокументу(ДокументДД), списокВидовДоговоров);
