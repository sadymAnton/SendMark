...
Процедура КнопкаЕщеСоздатьДокумент(Команда)
	...
	массивКонтекстаДляОбработки = МассивСсылокДокументов_ДД_В_ВыделенныхСтроках();
	
	НеобходимВызовФормыПостОбработки = Ложь;
	МассивДокументовДляПостОбработки = Неопределено;
	СоздатьДокументыНаСервере(массивКонтекстаДляОбработки, НеобходимВызовФормыПостОбработки, МассивДокументовДляПостОбработки);
	
	Если НеобходимВызовФормыПостОбработки Тогда

		ОткрытьФорму("ВнешняяОбработка.ПодключаемыйМодуль.Форма.ФормаПостОбработки", Новый Структура("МассивДокументовДляПостОбработки", МассивДокументовДляПостОбработки));
...
Функция МассивСсылокДокументов_ДД_В_ВыделенныхСтроках()
	...
	Для Каждого ИндексВыбраннойСтроки Из Элементы.СписокДокументов.ВыделенныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.ДокументЭДО)
			...
			Результат.Добавить(Новый Структура("ДокументДД, ПакетДД, ДокументСсылкаДляГрупповойПостОбработки, ПараметрыДляГрупповойПостОбработки", ТекущаяСтрока.ДокументЭДО, ТекущаяСтрока.Пакет, Неопределено, Новый Структура));
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура СоздатьДокументыНаСервере(МассивДокументов, НеобходимВызовФормыПостОбработки, МассивДокументовДляПостОбработки)
	
	НеобходимВызовФормыПостОбработки = Ложь;
	
	МассивДокументовДляПостОбработки = Новый Массив;
	
	Для Каждого СтруктураДанных Из МассивДокументов Цикл
		
		РезультатФункции = ОбработкаОбъект().СоздатьИСопоставитьДокументДД(СтруктураДанных.ДокументДД, СтруктураДанных.ПакетДД, СтруктураДанных.ДокументСсылкаДляГрупповойПостОбработки, СтруктураДанных.ПараметрыДляГрупповойПостОбработки);
		Если СтруктураДанных.ПараметрыДляГрупповойПостОбработки.Свойство("НеобходимаОбработка") и СтруктураДанных.ПараметрыДляГрупповойПостОбработки.НеобходимаОбработка Тогда
			МассивДокументовДляПостОбработки.Добавить( 
				Новый Структура("ДокументСсылка,Параметры", 
					СтруктураДанных.ДокументСсылкаДляГрупповойПостОбработки,
					СтруктураДанных.ПараметрыДляГрупповойПостОбработки) 
			);
			НеобходимВызовФормыПостОбработки = Истина;
		КонецЕсли;
		ОбработкаОбъект().ОбработатьРезультатФункции(РезультатФункции);
