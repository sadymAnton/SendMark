...
Процедура СоздатьДокументыНаСервере(МассивДокументов, НеобходимВызовФормыПостОбработки, МассивДокументовДляПостОбработки)
	...
	Для Каждого СтруктураДанных Из МассивДокументов Цикл
		...
		ПараметрыДляГрупповойПостОбработки = Неопределено;
		
		РезультатФункции = ОбработкаОбъект().СоздатьИСопоставитьДокументДД(СтруктураДанных.ДокументДД, СтруктураДанных.ПакетДД, ПараметрыДляГрупповойПостОбработки);
		ОбработкаОбъект().ОбработатьРезультатФункции(РезультатФункции);
		
		// Если по данному документу необходима пост-обработка
		Если ПараметрыДляГрупповойПостОбработки <> Неопределено Тогда
			МассивДокументовДляПостОбработки.Добавить( 
				Новый Структура("ДокументДД,Параметры", 
					СтруктураДанных.ДокументДД,
					ПараметрыДляГрупповойПостОбработки) 
			);
